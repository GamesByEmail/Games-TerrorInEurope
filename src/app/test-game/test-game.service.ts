...
import { debounceTime, filter, ignoreElements, mergeMap, tap } from 'rxjs/operators';
...
export class TestGameService {
  ...
  constructor(private sbUrl: string, private game: IStateGame, private gameData: IGameData<any, any, any>[]) {
    ...
    return merge(
      ...
      this.sbWindows
        ...
        .pipe(tap(sbWindow => {
          ...
          sbWindow.postMessage({ type: "teamTitles", titles: this.game.teams.map(team => team.title) }, "*");
          this.sendCurrentStates(sbWindow);
        })),

      fromEvent<MessageEvent>(window, "message")
        .pipe(filter(message => message.data && (message.data.type === "goToMove")))
        .pipe(tap(message => this.game.server.goToMove(message.data.moveNumber, message.data.teamIndex)))
    )
      .pipe(ignoreElements());
  }
  ...
  public reset() {
    ...
    this.getWindowBuffer().forEach(sbWindow => {
      if (!sbWindow.closed)
        sbWindow.postMessage({ type: "teamTitles", titles: this.game.teams.map(team => team.title) }, "*");
    });
    this.game.server.playTest(this.gameData[this.index]);
