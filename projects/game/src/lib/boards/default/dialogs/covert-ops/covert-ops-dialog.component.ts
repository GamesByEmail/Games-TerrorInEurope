...
export class CovertOpsDialogComponent {
  ...
  constructor(public dialogRef: SvgDialogRef<ICovertOpsDialogResult | undefined>, @Inject(SVG_DIALOG_DATA) public data: ICovertOpsDialogData) {
    this.rolls = this.data.operative ? this.data.operative.getRolls() : undefined;
  }
  pieceHref(operative: boolean = false) {
    if (!operative)
      return this.data.token.svgId();
    return this.data.operative ? this.data.operative.meepleId : "";
  }
  ...
  dieStyle(index: number) {
    ...
    const miss = this.data.token.type === ETokenType.TRAP ? 0 : 3;
    const critical = this.data.operative ? (this.data.token.type === ETokenType.TRAP ? this.data.operative.strength : 6) : 7;
  ...
  evaluateToBoolean(value: any): boolean {
    if (typeof (value) === "string" && value)
      return ((this.data.operative && this.data.operative.id === value) || this.data.token.type === value);
    return value;
  }
  showFor(value: any) {
